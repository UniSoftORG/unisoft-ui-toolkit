...
const elementWithPermissions = (Component: () => JSX.Element, state: State<UserState> ) => {
  const Permissions = () => {
    // Login data added to props via redux-store (or use react context for example)
    if (!state.userData) return <></>;
    if (!state.userData.permissions) return <></>;

    const hasPermissions = !!state.userData && state.userData.permissions.length > 0;
...
export const pageWithPermissions = (Component: NextPage, state: State<UserState>) => {
  const PagePermissions = () => {
    ...
    const isAdmin =
      state.userData?.permissions[0].permission === "superAdmin";
